...
window.onload = () => {
  document.getElementById("clickme").onclick = function (e) {
    ...
    let ax = -50 + (y * 7) / 9 - 180.73;
    let az = 350 - (x * 7) / 9 - 50 - 27.659;
    const cam = document.querySelector("#camera");
    cam.setAttribute(
      "position",
      `${ax} ${cam.getAttribute("position").y} ${az}`
    );
  };
  AFRAME.registerComponent("showinfo", {
    ...
    init: function () {
      ...
      controlBtn.addEventListener("click", (ee) => {
        ...
        if (this.data.coordE && this.data.coordN) {
        ...
        if (this.data.terrainID) {
          infoText.terrainID += this.data.terrainID;
          infoArr.push(infoText.terrainID);
        }
        if (
          this.data.typeName &&
          this.data.chestRadius &&
          Number(this.data.chestRadius) >= 10
        ) {
        ...
        if (this.data.elevation) {
          infoText.elevation +=
            String(Math.round(1000 * Number(this.data.elevation)) / 1000) +
            " m";
          infoArr.push(infoText.elevation);
        }
        if (this.data.height) {
          infoText.height +=
            String(Math.round(1000 * Number(this.data.height)) / 1000) + " m";
          infoArr.push(infoText.height);
        }
        if (this.data.chestRadius) {
          infoText.chestRadius +=
            String(Math.round(1000 * Number(this.data.chestRadius)) / 1000) +
            " cm";
          infoArr.push(infoText.chestRadius);
        }
        if (this.data.floors) {
          infoText.floors += this.data.floors;
          infoArr.push(infoText.floors);
        }

        if (document.querySelector(".tarInfo"))
          document.querySelector(".tarInfo").remove();

        createInfoBoard(attPos, infoArr, this.data.isBD);
      });
    },
  });
...
function createInfoBoard(itemPos, textArr, isBD = false) {
  if (!isBD) {
    ...
    enti.setAttribute("position", {
      x: itemPos["x"] - 3 * Math.cos((Math.PI * userRot[1]) / 180),
      y: itemPos["y"] + 2,
      z: itemPos["z"] + 3 * Math.sin((Math.PI * userRot[1]) / 180),
    });
    ...
    enti.setAttribute("position", {
      x:
        document.querySelector("#camera").getAttribute("position").x -
        2.5 * Math.sin(0.6 + (Math.PI * userRot[1]) / 180) +
        180.73,
      y: itemPos["y"] + 1.7,
      z:
        document.querySelector("#camera").getAttribute("position").z -
        2.5 * Math.cos(0.6 + (Math.PI * userRot[1]) / 180) +
        27.659,
    });
...
function addModels(objArr, modelID, modelScale, parentNode) {
  for (let i = 0; i < objArr.length; i++) {
    ...
    if (modelID == "cloud") {
      ...
      modelentis.setAttribute(
        "position",
        objArr[i].AframeLocalX + " 0 " + objArr[i].AframeLocalZ
      );
      modelentis.setAttribute(
        "showInfo",
        `itemName: ${objArr[i].RefName}; elevation: ${
          objArr[i].Elevation
        }; terrainID: ${objArr[i].TerrainID};
      typeName: ${objArr[i].Types ? objArr[i].Types : ""}; height: ${
          objArr[i].TrueHeight ? objArr[i].TrueHeight : "0"
        }; chestRadius: ${objArr[i].ChestRadius ? objArr[i].ChestRadius : ""};
      coordE: ${objArr[i].E}; coordN: ${objArr[i].N}`
      );
      if (objArr[i].TrueHeight)
        modelentis.setAttribute(
          "scale",
          `${(modelScale * objArr[i].TrueHeight) / 5} ${
            (modelScale * objArr[i].TrueHeight) / 5
          } ${(modelScale * objArr[i].TrueHeight) / 5}`
        );
      else
